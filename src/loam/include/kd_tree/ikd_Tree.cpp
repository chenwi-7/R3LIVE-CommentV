...
/**
 * @note 寻找point在ikd树中的最近平面,并保存平面中的最近点,点与当前point的距离序列,等信息
 * 
 * @param point 当前操作点
 * @param k_nearest 默认=5
 * @param Nearest_Points 保存与操作点在平面中的最近点
 * @param Point_Distance 保存Nearest_Points与操作点的距离(下标对应)
 * @param max_dist 最远距离
 */ 
void KD_TREE::Nearest_Search(PointType point, int k_nearest, PointVector& Nearest_Points, vector<float> & Point_Distance, double max_dist){   
    MANUAL_HEAP q(2*k_nearest); // 通过堆(小根堆)的方式管理最近的10个点
    q.clear();
    vector<float> ().swap(Point_Distance);  // 清空vector Point_Distance
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){
        Search(Root_Node, k_nearest, point, q, max_dist);   // 搜索最近10个点放置在ikd树中
    } else {
        pthread_mutex_lock(&search_flag_mutex);
    ...
    int k_found = min(k_nearest,int(q.size())); //min(额定5个点, 实际找到的点数量)
    PointVector ().swap(Nearest_Points);
    vector<float> ().swap(Point_Distance);
    for (int i=0;i < k_found;i++){  // 正式将搜索到的点放置在Nearest_Points,对应距离存在Point_Distance中
...
/**
 * @note 在ikd树中搜索与point最近的10个点,将10个点放置在堆q中
 * 
 */ 
